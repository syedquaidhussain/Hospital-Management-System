...
def bookings(): 
    ...
    if current_user.usertype=="Admin":
        ...
        query=db.engine.execute(f"SELECT * FROM `doctors` WHERE doctorname = '{username}'")
        dept = ""
        for i in query:
            dept = (i['dept'])
        query=db.engine.execute(f"SELECT * FROM `patients` WHERE dept = '{dept}'")
        return render_template('booking.html',query=query)
    else:
...
def details():
    # posts=Trigr.query.all()
        # posts=db.engine.execute(f"SELECT * FROM `patients` where depth='{dept}'")
    # else:
    posts=db.engine.execute("SELECT * FROM `trigr`")
    print(f"SELECT * FROM `trigr`")
